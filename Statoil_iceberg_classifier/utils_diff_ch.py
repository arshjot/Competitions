...
def randomNoisy(im, u=0.5):
    if np.random.random() < u:
        ...
        if noise_typ == "s&p":
            ...
            coords_0 = [np.random.randint(0, i - 1, int(num_salt))
                  for i in image.shape]
            coords_1 = [np.random.randint(0, i - 1, int(num_salt))
                  for i in image.shape]
            out[0, coords_0] = np.random.randint(-20, 15)
            out[1, coords_1] = np.random.randint(-20, 15)

            # Pepper mode
            num_pepper = np.ceil(amount* image.size * (1. - s_vs_p))
            coords_0 = [np.random.randint(0, i - 1, int(num_pepper))
                  for i in image.shape]
            coords_1 = [np.random.randint(0, i - 1, int(num_pepper))
                  for i in image.shape]
            out[0, coords_0] = np.random.randint(-40, -27)
            out[1, coords_1] = np.random.randint(-40, -27)
            return out
...
def randomRotation(image, u=0.5):
    if np.random.random() < u:
        ...
        ims = []
        for im in image:
            ims.append(rotate(im, rot, mode='symmetric'))
        return np.stack(ims)
    return image
