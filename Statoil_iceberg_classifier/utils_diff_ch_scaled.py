...
def randomZoomOut(image, zoom_ratio=(-0.5, 0.5), u=0.5):
    if np.random.random() < u:
        ...
        if int(zoom)<0:
            ...
            ims = []
            for im in image:
                ims.append(cv2.resize(im[zoom//2:-zoom//2, zoom//2:-zoom//2], (height, width)))
            return np.stack(ims)
        ...
        ims = []
        for im in image:
            ims.append(cv2.resize(cv2.copyMakeBorder(im, int(zoom//2), int(zoom//2), int(zoom//2), int(zoom//2),
...
def randomShift(image, shift_ratio=(0, 0.3), u=0.5):
    if np.random.random() < u:
        ...
        ims = []
        for im in image:
            ims.append(cv2.resize(im[0:width-lr_shift, 0:height-tb_shift], (height, width)))
        return np.stack(ims)
